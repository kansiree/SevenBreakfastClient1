	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if ((minDateTime != null && dateTime.isBefore(minDateTime))
			...
			if (CaldroidFragment.disabledBackgroundDrawable == -1) {
				cellView.setBackgroundResource(R.drawable.calendar_disable_cell);
			} else {
				cellView.setBackgroundResource(CaldroidFragment.disabledBackgroundDrawable);
			}

			if (dateTime.equals(getToday())) {
				cellView.setBackgroundResource(R.drawable.calendar_blue_sky_border_gray_bg);
			}
		} else {
		...
		if (shouldResetDiabledView && shouldResetSelectedView) {
			// Customize for today
			if (dateTime.equals(getToday())) {
				cellView.setBackgroundResource(R.drawable.calendar_blue_sky_border);
			} else {
